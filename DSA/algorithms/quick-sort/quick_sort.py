...
def quick_sort_3(arr):
    ...
    # picking a random pivot every time

    arr_len = len(arr)
    pivot_index = random.randint(0, arr_len-1)
    ...
    # # the pivot should not be included in the subarrays
    # less_arr = [x for i, x in enumerate(arr) if x <= pivot and i != pivot_index]
    # greater_arr = [x for i, x in enumerate(arr) if x >pivot and i != pivot_index]

    return quick_sort_3(less_arr) + [pivot] + quick_sort_3(greater_arr)
...
# ***************************************************************************
# comparing quick sort with first item as pivot 
# vs quick sort with random item as pivot
random_list = random.choices(range(-100, 1000000), k=6000000)
tic = time.perf_counter()
quick_sort_2(random_list)
toc = time.perf_counter()
print(f"sorted with first item as pivot in {toc - tic:0.4f} seconds")

tic = time.perf_counter()
quick_sort_3(random_list)
toc = time.perf_counter()
print(f"sorted with random item as pivot in {toc - tic:0.4f} seconds")
